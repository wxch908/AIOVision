...
using static AIOVision.MiniDump;

namespace AIOVision
{
    public class FunctionStack
    {
        public static Dictionary<object, Func<object, Task<bool>>> funcDic = new Dictionary<object, Func<object, Task<bool>>>();
        public static void Register(string key, Func<object, Task<bool>> func)
        {
            funcDic.Add(key, func);
        }
        public static async Task<bool> ExecuteAsync(string key, object param)
        {
            return await funcDic[key].Invoke(param);
        }
    }
    ...
    public class CommonBase
    {
        ...
        {
            Project prj = Solution.Ins.g_ProjectList.Find(c => c.ProjectInfo.m_ProjectName == SelectProName);
            if (prj != null)
            {
                Solution.Ins.Cur_Project = prj;
                Solution.Ins.Cur_Project.CurModuleID = prj.ProjectInfo.m_ProjectID;//2023.1.28对应流程修改后，点击流程UI不切换的Bug
