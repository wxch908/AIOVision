...
namespace AIOVision
{
    ...
    public class Project : ProjectThread, ICloneable
    {
        ...
        {
            //查询流程
            int index = Solution.Ins.g_ProjectList.FindIndex(c => c.ProjectInfo.m_ProjectID == ProjectInfo.m_ProjectID);

            //根据流程获取值
            return Solution.Ins.g_ProjectList[index].m_Var_List.Find(c => c.m_DataName == Link_data_Var.m_DataName &&
        ...
        public DataVar GetCurLocalVarValue(DataVar Link_data_Var)
        {
            ...
            return Solution.Ins.Cur_Project.m_Var_List.Find(c => c.m_DataName == Link_data_Var.m_DataName &&
            c.m_DataTip == Link_data_Var.m_DataTip);
        ...
        public void UnitProcess()
        {
            ...
            {
                foreach (var item in Solution.Ins.g_ProjectList)
                {
                    //执行流程
            ...
            //调用时执行并且是当前流程，委托刷新UI
            if (ProjectInfo.m_Execution == Execution.调用时执行 && Solution.Ins.Cur_Project == this)
            {
                DataEventChange.SingleProjectChangedEnabelData = false;
            }
            ...
            while (_threadStatus)
            {
                if (_threadStatus)
                {
                    try
                    {
                        ...
                        {
                            //刷新选中的流程列表,清除流程的显示
                            if (Solution.Ins.Cur_Project != null && Solution.Ins.Cur_Project.CurModuleID == this.CurModuleID)
                            {
                ...
                {
                    int index = Solution.Ins.g_ProjectList.FindIndex(c => c.ProjectInfo.m_ProjectID == ProjectInfo.m_ProjectID);
                    Thread.Sleep(Solution.Ins.SysInterval + index * 5);
                }
                else
                {
                    Thread.Sleep(Solution.Ins.SysInterval);
                }
            ...
            if (run != RunMode.None && Solution.Ins.Cur_Project == this)//2023.6.26添加
            {
                DataEventChange.SingleProjectChangedEnabelData = true;
            }
        ...
        private void Contrast()
        {
            ...
            if (!status)
            {
                DataEventChange.ProjectChangedEnabelData = true;
                Solution.Ins.g_SysStatus.m_RunMode = RunMode.None;
            }
